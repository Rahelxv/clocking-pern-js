...
const Stopwatch = ({ fDeleted, name, initialTime, onTimeUpdate }) => {
  const [isRunning, setIsRunning] = useState(false);
  const [time, setTime] = useState(initialTime); // Gunakan initialTime dari props
  ...
  useEffect(() => {
    if (isRunning) {
      ...
      intervalRef.current = setInterval(() => {
        ...
        setTime(newTime);
        onTimeUpdate(newTime); // Kirim waktu terbaru ke context
      }, 10);
    } else {
